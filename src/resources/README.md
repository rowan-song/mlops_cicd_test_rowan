...
ML Resource Configurations in this directory:
 ...
 - monitoring resource and workflow (`mlops_poc/resources/monitoring-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `mlops_poc/databricks.yml` and `mlops_poc/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops_poc-model     | /dev-mlops_poc-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops_poc-model | /staging-mlops_poc-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops_poc-model    | /prod-mlops_poc-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops_poc-model    | /test-mlops_poc-experiment    |
...
Following this, update the batch_inference_job base parameters in `mlops_poc/resources/batch-inference-workflow-resource.yml` to pass
...
The example above defines a Databricks job with name `${bundle.target}-mlops_poc-batch-inference-job`
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops_poc-batch-inference-job`.
...
`test-mlops_poc-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops_poc-batch-inference-job`.
